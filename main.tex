...
\begin{tabular}{@{}l@{\hspace{0.2cm}}
  ...
  @{\hspace{0.2cm}}l@{}}
    [Nom Prénom 1] &
    \dotfill &
    [Titre/Qualité 1] \\[0.2cm]
    [Nom Prénom 2] &
    \dotfill &
    [Titre/Qualité 2] \\[0.2cm]
    [Nom Prénom 3] &
    \dotfill &
    [Titre/Qualité 3] \\[0.2cm]
    [Nom Prénom 4] &
    \dotfill &
    [Titre/Qualité 4] \\
\end{tabular}
...
%
    ...
    \caption[Organigramme de Capgemini Engineering Maroc]{Organigramme structurel et op\'erationnel de Capgemini Engineering Maroc, d\'etaillant l'arborescence depuis la Direction des Op\'erations jusqu'au d\'epartement Product \& Systems Engineering, abritant l'\'equipe sp\'ecialis\'ee en Validation OTA pour Stellantis.}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        node distance=1.5cm and 2cm,
        every node/.style={font=\sffamily\small, align=center},
        cloud/.style={ellipse, draw, fill=blue!10, minimum width=3cm, minimum height=1.5cm, drop shadow, font=\sffamily\small\bfseries},
        gateway/.style={rectangle, draw, fill=yellow!10, rounded corners, minimum width=3cm, minimum height=1.2cm, drop shadow, font=\sffamily\small\bfseries},
        ecu/.style={rectangle, draw, fill=green!10, rounded corners, minimum width=2.5cm, minimum height=1cm, drop shadow, font=\sffamily\small},
        bus/.style={rectangle, draw, fill=gray!20, minimum width=8cm, minimum height=0.6cm, drop shadow, font=\sffamily\small\bfseries},
        zone/.style={draw, dashed, rounded corners, inner sep=15pt, fill=#1, fill opacity=0.05},
        arrow/.style={->, >=stealth, thick},
        wireless/.style={->, >=stealth, thick, dashed, color=blue!80},
        attack/.style={->, >=stealth, ultra thick, color=red!90}
    ]
%
    % Nodes
    \node[cloud] (backend) at (2, 2) {Cloud Stellantis\\(Serveurs OTA)};
    \node[rectangle, draw, rounded corners, fill=red!10, minimum width=2.5cm, minimum height=1cm, drop shadow, font=\sffamily\small\bfseries] (hacker) at (-3, 2) {Attaquant\\(Spoofing/MiTM)};
    
    \node[gateway] (tcu) at (0, -1.5) {Unité Télématique (TCU)\\(Passerelle 4G/5G/Wi-Fi)};
    \node[gateway] (ivi) at (5.5, -1.5) {Calculateur IVI\\(Infodivertissement)};
    
    \node[bus] (can) at (2.75, -3.5) {Bus Réseau Véhicule Critique (CAN / Automotive Ethernet)};
    
    \node[ecu] (ecu_mot) at (-1, -5) {ECU Moteur\\(Powertrain)};
    \node[ecu] (ecu_frein) at (2.25, -5) {ECU Freinage\\(Châssis)};
    \node[ecu] (ecu_ad) at (5.5, -5) {ECU Autonome\\(ADAS)};

    % Grouping borders
    \draw[zone=blue] (-4.5, -6) rectangle (8.5, -0.5);
    \node[below right, font=\sffamily\small\bfseries, color=blue!80] at (-4.5, -0.5) {Architecture Interne du Véhicule (Isolée)};
    \draw[zone=gray] (-4.5, 0.5) rectangle (8.5, 3.5);
    \node[below right, font=\sffamily\small\bfseries, color=gray!80] at (-4.5, 3.5) {Réseau Externe (Vulnérable)};

    % Connections
    \draw[wireless] (backend) -- (tcu) node[midway, right, font=\sffamily\scriptsize, color=black] {Mise à jour OTA\\(Canal sans fil)};
    \draw[attack] (hacker) -- (tcu) node[midway, sloped, above, font=\sffamily\tiny, color=red!90] {Injection de payload malveillant};
    
    \draw[arrow] (tcu) -- (ivi) node[midway, above, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {Réseau Interne\\(Vulnérable si compromis)};
    
    \draw[arrow] (tcu) -- (tcu |- can.north);
    \draw[arrow] (ivi) -- (ivi |- can.north);
    
    \draw[arrow] (can.south -| ecu_mot) -- (ecu_mot);
    \draw[arrow] (can.south -| ecu_frein) -- (ecu_frein);
    \draw[arrow] (can.south -| ecu_ad) -- (ecu_ad);
%
    \end{tikzpicture}
    }
    \caption[Architecture de communication OTA et menaces]{Architecture de communication OTA (Over-The-Air) et propagation potentielle des menaces vers les systèmes critiques du véhicule.}
    \label{fig:architecture_ota}
\end{figure}

\begin{table}[htbp]
\centering
\caption[Typologie des cybermenaces ciblant les calculateurs d'infodivertissement]{Typologie des cybermenaces ciblant les calculateurs d'infodivertissement (ECU)}
...
\begin{table}[htbp]
\centering
\caption[Comparaison des capacités de détection]{Comparaison des capacités de détection : Approche Classique vs Architecture Cyber-MAS}
\label{tab:comparaison_siem_mas}
\vspace{0.2cm}
\begin{tabular}{|>{\raggedright\arraybackslash}p{4.5cm}|>{\raggedright\arraybackslash}p{4.5cm}|>{\raggedright\arraybackslash}p{5cm}|}
\hline
...
%
    ...
    \caption[Diagramme de Gantt du projet]{Diagramme de Gantt schématisant l'enchaînement des itérations Agile Scrum sur les 16 semaines du projet.}
    \label{fig:gantt_chart}
\end{figure}

\begin{table}[htbp]
\centering
\caption[Découpage du projet selon la méthodologie Agile Scrum]{Découpage du projet selon la méthodologie Agile Scrum}
\label{tab:planning_agile}
\vspace{0.2cm}
\begin{tabular}{|l|>{\raggedright\arraybackslash}p{8cm}|c|}
\hline
...
%
    ...
    \caption[Comparaison Black Box (ML) vs Glass Box (LLM)]{Comparaison visuelle entre l'approche \textit{Black Box} du Machine Learning classique (gauche) et l'approche \textit{Glass Box} du LLM avec raisonnement par Cha\^ine de Pens\'ee (droite).}
...
\begin{table}[htbp]
\centering
\caption[Comparaison des paradigmes d'Intelligence Artificielle en cybersécurité]{Comparaison des paradigmes d'Intelligence Artificielle en cybersécurité}
\label{tab:comparaison_ia}
\vspace{0.2cm}
\begin{tabular}{|>{\raggedright\arraybackslash}p{3.5cm}|>{\raggedright\arraybackslash}p{5cm}|>{\raggedright\arraybackslash}p{5.5cm}|}
\hline
...
%
    \node[prop=green!15, above=2.5cm of agent] (auto) {\textbf{Autonomie}\\Contr\^ole ind\'ependant};
    \node[prop=cyan!15, right=3cm of agent] (react) {\textbf{R\'eactivit\'e}\\Perception temps r\'eel};
    \node[prop=orange!15, below=2.5cm of agent] (proact) {\textbf{Proactivit\'e}\\Initiative d'enrichissement};
    \node[prop=purple!15, left=3cm of agent] (social) {\textbf{Sociabilit\'e}\\D\'el\'egation inter-agents};
%
...
%
    ...
    \caption[Propriétés d\'un agent intelligent (Wooldridge)]{Les quatre propri\'et\'es fondamentales d'un agent intelligent selon Wooldridge et Jennings \cite{wooldridge_mas}, appliqu\'ees aux agents cognitifs de l'architecture Cyber-MAS.}
...
\begin{table}[htbp]
\centering
\caption[Analyse comparative des approches d'implémentation Multi-Agents]{Analyse comparative des approches d'implémentation Multi-Agents}
\label{tab:frameworks_mas}
\vspace{0.2cm}
\begin{tabular}{|>{\raggedright\arraybackslash}p{3.5cm}|>{\raggedright\arraybackslash}p{3cm}|>{\raggedright\arraybackslash}p{7cm}|}
\hline
...
\begin{table}[htbp]
\centering
\caption[Catégorisation des sources de Threat Intelligence]{Catégorisation des sources de Threat Intelligence (CTI) intégrables}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        node distance=1.5cm and 2cm,
        every node/.style={font=\sffamily\small, align=center},
        box/.style={rectangle, draw, rounded corners, minimum width=3cm, minimum height=1.2cm, drop shadow},
        db/.style={cylinder, draw, shape border rotate=90, aspect=0.25, fill=gray!10, minimum height=1.8cm, minimum width=2cm, drop shadow, align=center, font=\sffamily\small\bfseries},
        arrow/.style={->, >=stealth, thick},
    ...
    % Zone Data Ingestion
    \draw[zone=blue] (-4.5, -4) rectangle (1.5, 4) node[above left, opacity=1, font=\sffamily\small\bfseries, color=blue!80] {1. Phase d'Ingestion (Offline)};

    \node[box, fill=blue!10] (docs) at (-1.5, 2.5) {Documents sources\\(Rapports, CVE, MITRE)};
    \node[box, fill=blue!15] (embed1) at (-1.5, 0) {Modèle d'Embedding\\(Vectorisation)};
    
    \draw[arrow] (docs) -- (embed1);

    % Central DB
    \node[db, fill=purple!10] (vdb) at (1.5, -2.5) {Base de Données\\Vectorielle\\(Qdrant)};
    \draw[arrow] (embed1) -- (vdb) node[midway, left, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {Stockage des vecteurs};

    % Zone Retrieval & Generation
    \draw[zone=green] (2, -4) rectangle (9.5, 4) node[above left, opacity=1, font=\sffamily\small\bfseries, color=green!80] {2. Phase d'Inférence (Online)};

    \node[box, fill=orange!15] (query) at (5.5, 2.5) {Requête Utilisateur / Agent\\(Alerte JSON)};
    \node[box, fill=blue!15] (embed2) at (5.5, 0) {Modèle d'Embedding\\(Vectorisation de la requête)};
    
    \draw[arrow] (query) -- (embed2);
    
    \node[box, fill=yellow!15] (search) at (5.5, -2.5) {Recherche de Similarité\\(K-Nearest Neighbors)};
    
    \draw[arrow] (embed2) -- (search);
    \draw[dashedarrow] (vdb) -- (search) node[midway, above, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {Top-K contextes};

    \node[box, fill=green!15, minimum width=3.5cm] (llm) at (5.5, -5.5) {Grand Modèle de Langage (LLM)\\(Génération augmentée)};
    \node[box, fill=red!10] (output) at (5.5, -7.5) {Réponse Contextualisée\\(Verdict XAI)};
    
    \draw[arrow] (search) -- (llm) node[midway, right, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {Requête + Contexte};
    \draw[arrow] (llm) -- (output);

    \end{tikzpicture}
    }
    \caption[Schéma théorique du paradigme RAG]{Sch\'ema th\'eorique du paradigme RAG (\textit{Retrieval-Augmented Generation}) en cinq \'etapes s\'equentielles, de la requ\^ete initiale jusqu'\`a la g\'en\'eration du verdict augment\'e.}
...
%
    ...
    \draw[arrow] (user) -- (dispatch) node[midway, right, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {Soumet une requête};
%
...
%
    ...
    % --- Agent Email ---
    \node[agentbox, above left=2cm and 4.5cm of dispatch] (box_email) {};
    \node[agentlabel, anchor=south] at ([yshift=-0.3cm]box_email.north) {Agent Email};
    \node[module] at ([yshift=1.2cm]box_email.center) (llm_email) {\textbf{LLM} (Raisonnement)};
    \node[module] at (box_email.center) (tools_email) {\textbf{Tools} (Triggers)};
    \node[module] at ([yshift=-1.2cm]box_email.center) (mem_email) {\textbf{Mémoire} (RAG)};
%
    % --- Agent Log ---
    ...
    \node[module] at ([yshift=1.2cm]box_log.center) (llm_log) {\textbf{LLM} (Raisonnement)};
    \node[module] at (box_log.center) (tools_log) {\textbf{Tools} (Triggers)};
    \node[module] at ([yshift=-1.2cm]box_log.center) (mem_log) {\textbf{Mémoire} (RAG)};
%
    % --- Agent IP ---
    \node[agentbox, above right=2cm and 4.5cm of dispatch] (box_ip) {};
    \node[agentlabel, anchor=south] at ([yshift=-0.3cm]box_ip.north) {Agent Analyseur IP};
    \node[module] at ([yshift=1.2cm]box_ip.center) (llm_ip) {\textbf{LLM} (Raisonnement)};
    \node[module] at (box_ip.center) (tools_ip) {\textbf{Tools} (Triggers)};
    \node[module] at ([yshift=-1.2cm]box_ip.center) (mem_ip) {\textbf{Mémoire} (RAG)};
%
    % ===== Arrows from dispatcher to agents =====
    \draw[arrow] (dispatch.north) -- (box_log.south) node[midway, right, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {Logs Syslog};
...
%
    ...
    \draw[arrow] (box_email.north) ++(0,1.0) -- (corr.south west);
    \draw[arrow] (box_log.north) ++(0,1.0) -- (corr.south);
    \draw[arrow] (box_ip.north) ++(0,1.0) -- (corr.south east);
%
...
%
    ...
    \caption[Diagramme d'architecture globale du système distribué Cyber-MAS]{Diagramme d'architecture globale du système distribué Cyber-MAS, illustrant le flux asynchrone depuis l'utilisateur jusqu'au Corrélateur Global, en passant par le Dispatcher et les agents cognitifs dotés chacun de modules LLM, Tools et Mémoire vectorielle (RAG).}
...
%
    % Messages
    \draw[msg] (0, -1) -- (3.5, -1) node[midway, above, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {POST /api/analyse};
    \draw[msg] (3.5, -1.8) -- (7, -1.8) node[midway, above, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {async: logs};
    \draw[msg] (3.5, -2.3) -- (10.5, -2.3) node[midway, above, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {async: IP};
%
    \draw[msg] (7, -3.5) -- (10.5, -3.5) node[midway, above, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {Trigger IP check};
    \draw[rmsg] (10.5, -4.2) -- (7, -4.2) node[midway, above, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {reputation data};
%
    \draw[rmsg] (7, -5.5) -- (14, -5.5) node[midway, above, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {JSON verdict (log)};
    \draw[rmsg] (10.5, -6.2) -- (14, -6.2) node[midway, above, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {JSON verdict (ip)};
%
    \draw[msg] (14, -7) -- (17.5, -7) node[midway, above, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {query\_memory()};
    \draw[rmsg] (17.5, -7.7) -- (14, -7.7) node[midway, above, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {historical matches};
%
    \draw[rmsg] (14, -8.8) -- (0, -8.8) node[midway, above, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {SSE: Alerte XAI + MITRE};
%
    \draw[msg] (14, -9.5) -- (17.5, -9.5) node[midway, above, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {store\_report()};
%
    \end{tikzpicture}
    }
    \caption[Diagramme de séquence UML de l\'analyse multi-agents]{Diagramme de s\'equence UML mod\'elisant l'orchestration compl\`ete d'une analyse multi-agents, du \textit{Dispatcher} jusqu'\`a la restitution de l'alerte XAI via SSE.}
...
\begin{table}[htbp]
\centering
\caption[Spécification stricte du contrat d'interface JSON inter-agents]{Spécification stricte du contrat d'interface JSON inter-agents}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        node distance=1.5cm and 2cm,
        every node/.style={font=\sffamily\small, align=center},
        box/.style={rectangle, draw, rounded corners, minimum width=2.5cm, minimum height=1cm, drop shadow},
        db/.style={cylinder, draw, shape border rotate=90, aspect=0.25, minimum height=1.5cm, minimum width=2cm, drop shadow},
        llm/.style={circle, draw, minimum size=3.5cm, font=\sffamily\small\bfseries, drop shadow, fill=purple!10},
        arrow/.style={->, >=stealth, thick},
        dottedarrow/.style={->, >=stealth, thick, dashed}
    ]

    % Central LLM "Brain"
    \node[llm] (brain) at (0, 0) {Cerveau LLM\\(Orchestrateur)};

    % Inputs & Outputs
    \node[box, fill=green!10] (input) at (-5, 0) {Alerte Brute\\(JSON)};
    \node[box, fill=red!10] (output) at (5, 0) {Verdict Structuré\\\& Explication (XAI)};

    % Memory & Tools
    \node[db, fill=gray!20] (memory) at (0, 4) {Mémoire à long terme\\(Qdrant Vector DB)};
    \node[box, fill=yellow!10] (tools) at (0, -4) {Outils Externes\\(Triggers API, NIST)};

    % Interaction Loops
    \draw[arrow] (input) -- (brain) node[midway, above, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {Ingestion};
    \draw[arrow] (brain) -- (output) node[midway, above, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {Décision};

    \draw[->, >=stealth, thick, color=blue!80, bend left=20] (brain) to node[midway, right, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {Recherche de similarité} (memory);
    \draw[->, >=stealth, thick, color=blue!80, bend left=20] (memory) to node[midway, left, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {Contexte injecté (RAG)} (brain);

    \draw[->, >=stealth, thick, color=orange!80, bend left=20] (brain) to node[midway, left, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {Appel d'outil (Action)} (tools);
    \draw[->, >=stealth, thick, color=orange!80, bend left=20] (tools) to node[midway, right, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {Résultat (Observation)} (brain);

    % Cycle icon logic
    \node[font=\sffamily\large\bfseries, color=purple] at (0, 1.2) {\textbf{Réflexion}};
    
    \end{tikzpicture}
    }
    \caption[Dataflow du cycle cognitif d\'un agent]{Diagramme de flux de données (Dataflow) modélisant le cycle cognitif interne d'un agent spécialisé.}
    \label{fig:dataflow_agent}
\end{figure}

\begin{table}[htbp]
\centering
\caption[Cartographie des agents cognitifs de l'architecture Cyber-MAS]{Cartographie des agents cognitifs de l'architecture Cyber-MAS}
\label{tab:cartographie_agents}
\vspace{0.2cm}
\begin{tabular}{|l|>{\raggedright\arraybackslash}p{4cm}|>{\raggedright\arraybackslash}p{3.5cm}|>{\raggedright\arraybackslash}p{4.5cm}|}
\hline
...
%
    ...
    \node[mystep, fill=red!10, below=1.5cm of llm] (answer) {R\'eponse Experte\\(Verdict XAI)};
%
    % Background Docs entering DB
    \node[doc, above=1cm of vdb] (docs) {Base de connaissances\\(Rapports SOC passés)};
%
    % Arrows
    \draw[dashedarrow] (docs) -- (vdb) node[midway, right, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {Indexation};
    \draw[arrow] (query) -- (embed);
    \draw[arrow] (embed) -- (vdb) node[midway, above, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {Vecteur sémantique};
    \draw[arrow] (vdb) -- (retrieve) node[midway, right, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {Recherche (Cosinus)};
...
%
    ...
    \caption[Cinématique de fonctionnement du RAG]{Cinématique de fonctionnement du paradigme RAG (Génération Augmentée par la Recherche) évitant les hallucinations du modèle.}
...
\begin{table}[htbp]
\centering
\caption[Comparaison des solutions de stockage vectoriel intégrées à...]{Comparaison des solutions de stockage vectoriel intégrées à Cyber-MAS}
...
\begin{table}[htbp]
\centering
\caption[Spécifications de l'environnement matériel et logiciel]{Spécifications de l'environnement matériel et logiciel}
...
\begin{figure}[H]
    ...
    \caption[Interface principale du Dashboard Cyber-MAS]{Interface principale du Dashboard Cyber-MAS : Panneau d'analyse des vecteurs de menaces.}
...
\begin{figure}[H]
    ...
    \caption[Vue de monitoring du pipeline d'exécution multi-agents en...]{Vue de monitoring du pipeline d'exécution multi-agents en temps réel.}
...
\begin{figure}[H]
    ...
    \caption[Vue de la matrice MITRE ATT\&CK intégrée au...]{Vue de la matrice MITRE ATT\&CK intégrée au Dashboard Cyber-MAS, affichant les tactiques et techniques détectées.}
...
\begin{figure}[H]
    ...
    \caption[Interface de l'Agent Corrélateur]{Interface de l'Agent Corrélateur, illustrant la fusion sémantique des événements de sécurité.}
...
\begin{figure}[H]
    ...
    \caption[Panneau de gestion et de triage des alertes...]{Panneau de gestion et de triage des alertes de sécurité pour les analystes SOC.}
...
\begin{figure}[H]
    ...
    \caption[Tableau de bord des statistiques globales de détection...]{Tableau de bord des statistiques globales de détection et métriques du système (MTTD, etc.).}
...
\begin{figure}[H]
    ...
    \caption[Interface de Threat Hunting]{Interface de Threat Hunting (Recherche de Menaces) permettant d'interroger la base vectorielle Qdrant.}
...
\begin{figure}[H]
    ...
    \caption[Vue détaillée de l'état]{Vue détaillée de l'état, de la configuration et des journaux individuels des agents LLM.}
    \label{fig:dashboard_agents}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{media/screenshot_timeline.png}
    \caption[Vue Timeline d'Attaque]{Représentation chronologique de l'attaque modélisée selon les étapes de la Kill-Chain MITRE ATT\&CK.}
    \label{fig:dashboard_timeline}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{media/screenshot_logs.png}
    \caption[Flux de Logs en Direct]{Visualisation du flux des événements système et réseau en temps réel.}
    \label{fig:dashboard_logs}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{media/screenshot_history.png}
    \caption[Historique des Analyses]{Historique complet des analyses précédentes stocké de manière persistante (SQLite).}
    \label{fig:dashboard_history}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{media/screenshot_health.png}
    \caption[Diagnostic de Santé du Système]{Panneau de supervision vérifiant l'état des APIs externes et de la base vectorielle Qdrant.}
    \label{fig:dashboard_health}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption[Exécution de Cyber-MAS en mode console]{Exécution de Cyber-MAS en mode console (CLI), affichant le statut de l'analyse et la matrice des résultats.}
    \label{fig:cli_mode}
\end{figure}

Outre l'analyse complète démontrée ci-dessus, le mode CLI offre plusieurs commandes spécialisées pour différents cas d'usage :


\textbf{Vérification de l'Environnement (Setup Check)}
\begin{lstlisting}[language=bash, basicstyle=\footnotesize\ttfamily\color{codeid}, caption={Commandes de vérification de l\'installation et de la connectivité API.}]
$ python main.py --check

[SYSTEM] Lancement du diagnostic...
[+] Python 3.11+ détecté
[+] Dépendances installées (requirements.txt)
[+] Base vectorielle locale Qdrant : OK
[+] Fichier .env chargé avec succès

$ python tools/llm_client.py

[GROQ API] Test de connectivité...
[+] Authentification réussie (Modèle: LLaMA 3.3-70B)
[+] Latence: 450ms
[+] Status: Prêt pour l'inférence.
\end{lstlisting}


\textbf{Analyse Ciblée (Single-Agent)}
\begin{lstlisting}[language=bash, basicstyle=\footnotesize\ttfamily\color{codeid}, caption={Analyse isolée d\'un fichier suspect via l\'Agent Email.}]
$ python main.py --email suspicious.eml

[EMAIL AGENT] Démarrage de l'analyse...
[+] Extraction des en-têtes... OK (From: admin@paypal-update.com)
[+] Recherche de similarité (FAISS)... Trouvé 2 correspondances (Score: 0.89)
[+] Analyse LLM en cours...
=> Verdict : PHISHING (Risque: 0.95)
=> Raison  : Tentative de spoofing du domaine PayPal.
\end{lstlisting}

\textbf{Monitoring Continu (Watch Mode)}
\begin{lstlisting}[language=bash, basicstyle=\footnotesize\ttfamily\color{codeid}, caption={Surveillance en temps réel d\'un fichier de logs système.}]
$ python main.py --watch-log /var/log/auth.log

[MONITOR] Surveillance active sur /var/log/auth.log (Ctrl+C pour quitter)
[+] Nouvelle ligne détectée : "Failed password for root from 192.168.1.45"
[+] Nouvelle ligne détectée : "Failed password for root from 192.168.1.45"
[LOG AGENT] Anomalie détectée ! Déclenchement de l'analyse...
=> Verdict : CRITICAL (Brute-force SSH)
\end{lstlisting}

\textbf{Analyse Rapide (\textit{Inline Text})}
\begin{lstlisting}[language=bash, basicstyle=\footnotesize\ttfamily\color{codeid}, caption={Scan instantané d\'une adresse IP sans fichier source.}]
$ python main.py --ip-text "203.0.113.42"

[IP AGENT] Démarrage du scan sur 203.0.113.42...
[+] OS Detection... Linux 3.x
[+] Ports ouverts : 22 (SSH), 80 (HTTP)
[+] CVE trouvées : CVE-2021-3156 (Score CVSS: 8.1)
=> Verdict : VULNERABLE
\end{lstlisting}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.8\textwidth]{media/screenshot_pdf.png}}
    \caption[Extrait d\'un rapport d\'incident PDF]{Extrait de la première page d'un rapport d'incident généré automatiquement au format PDF par Cyber-MAS.}
...
\begin{lstlisting}[language=json, basicstyle=\footnotesize\ttfamily\color{codeid}, caption={Exemple de charge utile JSON générée par l\'Agent Corrélateur.}]
{ - Valid Accounts",
  "ioc": ["185.199.108.153"],
...
Pour quantifier l'apport opérationnel de Cyber-MAS, l'architecture a été confrontée à un jeu de données hybride de sécurité combinant le corpus SpamAssassin, les logs réseau CIC-IDS 2017 et des simulations de vulnérabilités \cite{sans_soc_kpi}. Nous avons comparé l'architecture distribuée à un pipeline classique (SIEM + Machine Learning de type \textit{Random Forest}).

\subsection{Évaluation Individuelle des Agents}
L'architecture reposant sur la modularité, chaque agent a été évalué de manière indépendante sur son domaine de spécialisation. Le Tableau~\ref{tab:agent_performance} présente la précision de détection, le taux de faux positifs (FPR) et le taux de succès de l'explicabilité (XAI).

\begin{table}[htbp]
\centering
\caption[Performances individuelles des agents]{Performances individuelles des agents cognitifs sur les jeux de données cibles.}
\label{tab:agent_performance}
\vspace{0.2cm}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Agent} & \textbf{Précision de Détection} & \textbf{Faux Positifs (FPR)} & \textbf{Succès Explicabilité} \\ \hline
\textbf{Agent Email (Phishing)} & 94.1 \% & 5.4 \% & 90.2 \% \\ \hline
\textbf{Agent Log Analyzer} & 91.8 \% & 8.6 \% & 85.1 \% \\ \hline
\textbf{Agent IP (Threat Intel)} & 92.5 \% & 4.2 \% & 93.2 \% \\ \hline
\end{tabular}
\end{table}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            legend style={at={(0.5,-0.25)}, anchor=north, legend columns=-1, draw=gray!40, fill=white},
            ylabel={Pourcentage (\%)},
            symbolic x coords={Agent Email, Agent Log, Agent IP},
            xtick=data,
            nodes near coords,
        ...
        \addplot[fill=blue!60!cyan, draw=none] coordinates {(Agent Email, 94.1) (Agent Log, 91.8) (Agent IP, 92.5)};
        \addplot[fill=red!50!orange, draw=none] coordinates {(Agent Email, 5.4) (Agent Log, 8.6) (Agent IP, 4.2)};
        \addplot[fill=green!60!teal, draw=none] coordinates {(Agent Email, 90.2) (Agent Log, 85.1) (Agent IP, 93.2)};
        \legend{Précision, FPR, Explicabilité (XAI)}
        \end{axis}
    \end{tikzpicture}
    \caption[Graphique des performances individuelles des agents]{Représentation graphique des performances de détection, du taux de faux positifs (FPR) et d'explicabilité pour chaque agent cognitif.}
    \label{fig:agent_performance_chart}
\end{figure}


\subsection{Évaluation Globale de la Chaîne (Pipeline)}
La véritable innovation de Cyber-MAS réside dans le \textit{Corrélateur Global}. Les résultats synthétisés dans le Tableau~\ref{tab:kpi_comparison} et la Figure~\ref{fig:kpi_chart} démontrent une amélioration drastique des opérations SOC.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            xtick=data,
            nodes near coords,
            every node near coord/.append style={font=\sffamily\small, color=black},
            nodes near coords align={vertical},
            bar width=1.5cm,
            ymin=0, ymax=115,
            ymajorgrids=true,
        ...
        \addplot[fill=gray!40, draw=none] coordinates {(Précision, 78.5) (Explicabilité (XAI), 0)};
        \addplot[fill=blue!60!cyan, draw=none] coordinates {(Précision, 93.6) (Explicabilité (XAI), 100)};
        \legend{SIEM + ML Classique, Cyber-MAS}
        \end{axis}
    \end{tikzpicture}
    \caption[Comparaison de précision et explicabilité]{Comparaison graphique de la précision et du taux d'explicabilité entre les systèmes classiques et Cyber-MAS.}
    \label{fig:kpi_chart}
\end{figure}

\begin{table}[htbp]
\centering
\caption[Comparaison des Indicateurs de Performance]{Comparaison des Indicateurs de Performance (KPIs) SOC Globaux.}
\label{tab:kpi_comparison}
\vspace{0.2cm}
\begin{tabular}{|l|c|c|>{\raggedright\arraybackslash}p{4cm}|}
\hline
\textbf{Indicateur Clé (KPI)} & \textbf{SIEM + ML Classique} & \textbf{Cyber-MAS} & \textbf{Justification Technique} \\ \hline
\textbf{Précision Globale} & 78.5 \% & \textbf{93.6 \%} & Corrélation sémantique inter-domaines évitant les erreurs de modèles isolés. \\ \hline
\textbf{Précision de Corrélation} & N/A & \textbf{87.0 \%} & Le LLM réussit à lier la bonne cause au bon événement. \\ \hline
\textbf{Réduction Faux Positifs} & Référence & \textbf{- 41.3 \%} & L'analyse contextuelle et le RAG éliminent le bruit de fond. \\ \hline
\textbf{Temps de Triage (Analyste)} & ~ 45 minutes & \textbf{- 38.5 \%} & Le rapport XAI accélère drastiquement la prise de décision. \\ \hline
\textbf{Confiance de l'Analyste} & Variable & \textbf{4.6 / 5} & Transparence totale du raisonnement (\textit{Chain-of-Thought}). \\ \hline
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption[Impact de l'In-Context Learning]{Impact de l'In-Context Learning (RAG) sur la réduction de la fatigue d'alerte}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \addplot[fill=red!50!orange, draw=none] coordinates {(Trafic normal, 2.4) (Scans internes, 18.5) (MAJ OTA, 8.5)};
        \addplot[fill=green!60!teal, draw=none] coordinates {(Trafic normal, 0.8) (Scans internes, 4.2) (MAJ OTA, 3.1)};
        \legend{Sans RAG, Avec RAG}
        \end{axis}
    \end{tikzpicture}
    \caption[Impact de la boucle d'apprentissage RAG sur la...]{Impact de la boucle d'apprentissage RAG sur la réduction du Taux de Faux Positifs (FPR) par scénario de test sur banc HIL.}
...
\section{Le "System Prompt" : Rôle et Contexte}
...
\begin{lstlisting}[basicstyle=\footnotesize\ttfamily\color{codeid}, breaklines=true, caption={Définition du rôle et du contexte industriel.}]
SYSTEM_PROMPT = """
...
\section{La technique du "Chain of Thought" (CoT)}
...
\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[basicstyle=\footnotesize\ttfamily\color{codeid}, breaklines=true, caption={Directive forçant le raisonnement par étapes (Chain of Thought).}]
COT_DIRECTIVE = """
...
\section{Formatage de sortie (Output Parsing)}
...
\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[basicstyle=\footnotesize\ttfamily\color{codeid}, breaklines=true, caption={Contrainte syntaxique pour la génération de JSON pur.}]
FORMAT_INSTRUCTIONS = """
...
\section{Les Agents Cognitifs (Système Multi-Agents)}
...
\subsection{L'Agent Log (Analyseur de trames ECU)}
...
\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Python, basicstyle=\footnotesize\ttfamily\color{codeid}, breaklines=true, caption={Extraction et analyse des codes d'erreur par l'Agent Log.}]
...
\subsection{L'Agent IP (Analyseur de réputation réseau)}
...
\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Python, basicstyle=\footnotesize\ttfamily\color{codeid}, breaklines=true, caption={Analyse de l'adresse source par l'Agent IP.}]
...
\subsection{L'Agent Corrélateur (Fusion et LLM)}
...
\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Python, basicstyle=\footnotesize\ttfamily\color{codeid}, breaklines=true, caption={Fusion des donnees et appel au LLM par le Correlateur.}]
...
\section{Les Outils Externes (Tools \& APIs)}
...
\subsection{Outil de Mémoire Vectorielle (RAG)}
...
\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Python, basicstyle=\footnotesize\ttfamily\color{codeid}, breaklines=true, caption={Outil RAG pour l'apprentissage en contexte.}]
...
\subsection{Outil de Threat Intelligence (CTI)}
...
\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Python, basicstyle=\footnotesize\ttfamily\color{codeid}, breaklines=true, caption={Outil d'interrogation des bases de reputation reseau.}]
...
\section{Mode Continu (Continuous Monitoring)}
...
\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Python, basicstyle=\footnotesize\ttfamily\color{codeid}, breaklines=true, caption={Boucle de surveillance asynchrone des logs HIL.}]
import asyncio
...
\section{Reporting et Notifications}
...
\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Python, basicstyle=\footnotesize\ttfamily\color{codeid}, breaklines=true, caption={Gestionnaire d'alertes et déclenchement de webhook.}]
...
\section{Dashboard Temps Réel (Server-Sent Events)}
...
\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Python, basicstyle=\footnotesize\ttfamily\color{codeid}, breaklines=true, caption={Endpoint SSE pour le flux de données en temps réel.}]
...
\section{Gestion des erreurs et Résilience (Error Handling)}
...
\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Python, basicstyle=\footnotesize\ttfamily\color{codeid}, breaklines=true, caption={Bloc try/except pour sécuriser l'appel au LLM.}]
...
\section{Log Brut Entrant (Simulation d'échec de mise à jour)}
...
\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=json, basicstyle=\footnotesize\ttfamily\color{codeid}, caption={Simulation d\'un événement de journalisation OTA (Format JSON).}]
{,
...
\section{Résultat Intermédiaire (Sortie de l'Agent IP)}
\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=json, basicstyle=\footnotesize\ttfamily\color{codeid}, caption={Résultat d\'analyse de l\'Agent IP renvoyé au Dispatcher.}]
{,
...
\section{Rapport XAI Final (Généré par le Corrélateur Cyber-MAS)}
Ce JSON final est poussé vers l'interface du SOC, offrant une lisibilité immédiate à l'analyste.

\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=json, basicstyle=\footnotesize\ttfamily\color{codeid}, caption={Alerte finale corrélée avec la matrice MITRE ATT\&CK.}]
{,
  "ioc": ["185.199.108.153"],
...
\begin{table}[htbp]
\centering
\caption[Matrice MITRE ATT\&CK appliquee aux systemes embarques et...]{Matrice MITRE ATT\&CK appliquee aux systemes embarques et a la Validation OTA}
\vspace{0.2cm}
...
\begin{lstlisting}[language=bash, basicstyle=\footnotesize\ttfamily\color{codeid}, caption={Commandes d\'installation et de préparation de l\'environnement virtuel.}]
...
\begin{lstlisting}[language=bash, basicstyle=\footnotesize\ttfamily\color{codeid}, caption={Configuration des variables d\'environnement (.env).}]
# --- Modeles IA ---
LLM_PROVIDER=groq
...
\section{Lancement de la Plateforme (Dashboard)}
...
\begin{lstlisting}[language=bash, basicstyle=\footnotesize\ttfamily\color{codeid}, caption={Lancement du serveur Uvicorn pour l\'API et le Dashboard.}]
...
\section{Lancement en Mode Console (Headless)}
...
\begin{lstlisting}[language=bash, basicstyle=\footnotesize\ttfamily\color{codeid}, caption={Commandes d\'exécution de Cyber-MAS en mode console (CLI).}]
