...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        every node/.style={font=\sffamily\small, align=center},
        cloud/.style={cloud, draw, cloud puffs=10, cloud puff arc=120, aspect=2, inner ysep=1em, fill=gray!10, drop shadow},
        gateway/.style={rectangle, draw, rounded corners, fill=red!10, minimum width=3cm, minimum height=1.2cm, drop shadow},
        bus/.style={rectangle, draw, fill=blue!10, minimum width=10cm, minimum height=0.6cm, drop shadow},
        ecu/.style={rectangle, draw, fill=green!10, minimum width=2.2cm, minimum height=1cm, drop shadow},
        hacker/.style={rectangle, draw, fill=black, text=white, rounded corners, minimum width=2.5cm, drop shadow},
        arrow/.style={->, >=stealth, thick},
        wireless/.style={->, >=stealth, thick, dashed, color=blue!80},
        attack/.style={->, >=stealth, ultra thick, red!80},
        zone/.style={draw, dashed, rounded corners, inner sep=15pt, fill=#1, fill opacity=0.05}
    ]

    % Zone backgrounds
    \draw[zone=gray] (-6, -0.5) rectangle (3, 7.5);
    \node[below right, font=\sffamily\small\bfseries, color=gray!80] at (-6, 7.5) {Réseau Externe (Vulnérable)};

    \draw[zone=blue] (-5, -6) rectangle (8.5, 1.5);
    \node[above right, font=\sffamily\small\bfseries, color=blue!80] at (-5, -6) {Architecture Interne (Isolée)};

    % Top Level
    \node[cloud] (cloud) at (0, 6) {Serveur Backend OTA\\(Constructeur automobile)};
    \node[hacker] (hacker) at (-4, 4) {Attaquant\\(Réseau Compromis)};
    
    % Middle Level
    \node[gateway] (tcu) at (0, 2) {Calculateur TCU\\(Télématique)};
    \node[gateway] (ivi) at (5.5, 0) {Calculateur IVI\\(Infodivertissement)};
    
    % Bus Level
    \node[bus] (can) at (2.75, -2.5) {Bus CAN / Ethernet embarqué};
    
    % ECU Level
    \node[ecu] (ecu1) at (-1, -4.5) {ECU Moteur};
    \node[ecu] (ecu2) at (2.75, -4.5) {ECU Freins};
    \node[ecu] (ecu3) at (6.5, -4.5) {ECU Direction};

    % Connections
    \draw[wireless] (cloud) -- (tcu) node[midway, right, font=\sffamily\scriptsize, color=black] {Mise à jour OTA\\(Canal sans fil)};
    \draw[attack] (hacker) -- (tcu) node[midway, sloped, above, font=\sffamily\tiny, color=red!90] {Injection malveillante};
    
    \draw[arrow] (tcu) -- (ivi) node[midway, above, fill=white, inner sep=2pt, font=\sffamily\scriptsize] {Réseau Interne};
    
    \draw[arrow] (tcu) -- (tcu |- can.north);
    \draw[arrow] (ivi) -- (ivi |- can.north);
    \draw[arrow] (can.south -| ecu1) -- (ecu1.north);
    \draw[arrow] (can.south -| ecu2) -- (ecu2.north);
    \draw[arrow] (can.south -| ecu3) -- (ecu3.north);

    \end{tikzpicture}
    }
    \caption[Architecture OTA]{Architecture type d'un système de mise à jour OTA et ses vecteurs d'attaque sur le réseau de bord.}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        agent/.style={circle, draw, minimum size=3cm, font=\sffamily\small\bfseries, drop shadow, fill=blue!10, align=center},
        prop/.style={rectangle, draw, rounded corners, minimum width=3.5cm, minimum height=1.2cm, align=center, font=\sffamily\small, drop shadow},
        arrow/.style={<->, >=stealth, thick, color=gray!70}
    ]

    \node[agent] (agent) {\textbf{Agent Cognitif}\\\textit{(Nœud de calcul)}};

    \node[prop, fill=green!15, above left=of agent] (auto) {\textbf{Autonomie}\\Contrôle indépendant};
    \node[prop, fill=cyan!15, above right=of agent] (react) {\textbf{Réactivité}\\Perception temps réel};
    \node[prop, fill=orange!15, below right=of agent] (proact) {\textbf{Proactivité}\\Initiative d'enrichissement};
    \node[prop, fill=purple!15, below left=of agent] (social) {\textbf{Sociabilité}\\Délégation inter-agents};
    ...
    \end{tikzpicture}
    \caption{Propriétés fondamentales d'un agent cognitif dans l'architecture Cyber-MAS.}
...
\vspace{0.2cm}
\begin{tabular}{|>{\raggedright\arraybackslash}p{4.5cm}|c|c|>{\raggedright\arraybackslash}p{4.5cm}|}
\hline
