...
% Numérotation en bas de page (ENSAJ)
\pagestyle{fancy}
\fancyhf{}
\fancyfoot[C]{\thepage}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        source/.style={cylinder, draw, shape border rotate=90, aspect=0.25, fill=gray!10, minimum height=1.5cm, minimum width=1.5cm, align=center, font=\sffamily\scriptsize, drop shadow},
        dispatcher/.style={rectangle, draw, rounded corners, fill=purple!10, minimum width=3cm, minimum height=1.2cm, align=center, font=\sffamily\small\bfseries, drop shadow},
        agent/.style={rectangle, draw, rounded corners, fill=blue!10, minimum width=2.5cm, minimum height=1.2cm, align=center, font=\sffamily\small, drop shadow},
        correlator/.style={rectangle, draw, rounded corners, fill=red!10, minimum width=3.5cm, minimum height=1.2cm, align=center, font=\sffamily\small\bfseries, drop shadow},
        db/.style={cylinder, draw, shape border rotate=90, aspect=0.25, fill=green!10, minimum height=1.5cm, minimum width=1.5cm, align=center, font=\sffamily\scriptsize, drop shadow},
        arrow/.style={->, >=stealth, thick},
        dashedarrow/.style={->, >=stealth, thick, dashed}
    ]
%
    % Input sources
    \node[source] (logs) {Logs ECU\\Syslog};
    \node[source, below=1cm of logs] (net) {Trafic\\Réseau};
    \node[source, below=1cm of net] (email) {Fichiers\\.eml};
%
    % Dispatcher
    \node[dispatcher, right=3cm of net] (dispatch) {Dispatcher asynchrone\\(FastAPI)};
%
    % Agents
    \node[agent, right=2.5cm of dispatch, yshift=2.5cm] (alog) {Agent Log};
    \node[agent, right=2.5cm of dispatch] (aip) {Agent IP};
    \node[agent, right=2.5cm of dispatch, yshift=-2.5cm] (aemail) {Agent Email};
%
    % Correlator & DB
    \node[correlator, right=3cm of aip] (corr) {Corrélateur Global\\\& Matrice MITRE};
    \node[db, above=1.5cm of corr] (qdrant) {Vector DB\\(Qdrant)};
%
    % End User
    \node[rectangle, draw, fill=orange!10, rounded corners, right=2cm of corr, align=center, font=\sffamily\small, drop shadow] (soc) {Dashboard SOC\\(SSE)};
%
    % Connections
    \draw[arrow] (logs) -- (dispatch.north west);
    \draw[arrow] (net) -- (dispatch.west);
    \draw[arrow] (email) -- (dispatch.south west);
%
    \draw[arrow] (dispatch.north east) -- (alog.west) node[midway, above, sloped, font=\sffamily\scriptsize] {JSON};
    \draw[arrow] (dispatch.east) -- (aip.west) node[midway, above, font=\sffamily\scriptsize] {JSON};
    \draw[arrow] (dispatch.south east) -- (aemail.west) node[midway, below, sloped, font=\sffamily\scriptsize] {JSON};
%
    \draw[arrow] (alog.east) -- (corr.north west);
    \draw[arrow] (aip.east) -- (corr.west);
    \draw[arrow] (aemail.east) -- (corr.south west);
%
    \draw[dashedarrow] (corr.east) -- (soc.west) node[midway, above, font=\sffamily\scriptsize] {Alerte XAI};
%
    % Memory connections
    \draw[<->, >=stealth, thick, color=blue!70] (alog.north east) -- (qdrant.south west);
    \draw[<->, >=stealth, thick, color=blue!70] (aip.north east) -- (qdrant.south);
    \draw[<->, >=stealth, thick, color=blue!70] (corr.north) -- (qdrant.south east);
%
    \end{tikzpicture}
    }
    \caption{Diagramme d'architecture globale du système distribué Cyber-MAS, illustrant le flux asynchrone des données et la corrélation multi-agents.}
